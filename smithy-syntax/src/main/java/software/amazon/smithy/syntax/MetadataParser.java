...
final class MetadataParser implements Parser {
    ...
    public TokenTree parse() {
        return tokenizer.withState(TreeType.METADATA_SECTION, () -> {
            while (tokenizer.isCurrentLexeme("metadata")) {
                tokenizer.withState(TreeType.METADATA_STATEMENT, () -> {
                    tokenizer.withState(TreeType.KEYWORD, tokenizer::next); // append metadata
